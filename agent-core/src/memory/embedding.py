"""Embedding client with pluggable providers (ported from agent-rag).

- pseudo:    deterministic hash vectors, no deps (tests/dev/degraded fallback).
...
Any init/embed failure degrades to pseudo so the memory pipeline keeps running
without a model (mirrors agent-rag's behavior and this repo's degrade-not-crash
convention).
"""
import hashlib
...
class Embedder:
    ...
    def _ensure_fastembed(self) -> bool:
        ...
        try:
            ...
            cache_dir = getattr(self.settings, "fastembed_cache_path", None) or None
